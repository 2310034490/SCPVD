...
def load_bigvul_dataset(file_path):
    """加载BigVul数据集（支持JSON格式）"""
    data = []
    with open(file_path, 'r', encoding='utf-8') as f:
        # 尝试按行读取，以支持jsonl格式，如果不是jsonl，则尝试一次性加载
        try:
            for line in f:
                data.append(json.loads(line))
        except json.JSONDecodeError:
            # 如果不是jsonl，则回退到一次性加载整个json文件
            f.seek(0) # 重置文件指针
            data = json.load(f)
    return data
...
def process_bigvul_dataset(input_file, output_dir):
    """处理bigvul数据集并保存为训练、验证和测试集"""
    # 创建输出目录
    os.makedirs(output_dir, exist_ok=True)

    # 加载数据集
    data = load_bigvul_dataset(input_file)
    ...
    # 保存分割后的数据集
    save_jsonl(processed_train_data, os.path.join(output_dir, 'train_bigvul.jsonl'))
    save_jsonl(processed_val_data, os.path.join(output_dir, 'valid_bigvul.jsonl'))
    save_jsonl(processed_test_data, os.path.join(output_dir, 'test_bigvul.jsonl'))
...
if __name__ == '__main__':
    input_file = '../dataset/data/MSR_data_cleaned.json'
    output_dir = '../dataset/data/bigvul'
    process_bigvul_dataset(input_file, output_dir)