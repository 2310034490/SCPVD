...
def load_reveal_dataset(file_path):
    """加载ReVeal数据集"""
...
def process_reveal_dataset(input_file, output_dir):
    """处理ReVeal数据集并保存为训练、验证和测试集"""
    # 创建输出目录
    os.makedirs(output_dir, exist_ok=True)
    
    # 加载数据集
    data = load_reveal_dataset(input_file)
    ...
    # 保存分割后的数据集
    save_jsonl(processed_train_data, os.path.join(output_dir, 'train_reveal.jsonl'))
    save_jsonl(processed_val_data, os.path.join(output_dir, 'valid_reveal.jsonl'))
    save_jsonl(processed_test_data, os.path.join(output_dir, 'test_reveal.jsonl'))
    ...
    print(f"测试集idx范围: {len(processed_train_data)+len(processed_val_data)}-{len(processed_train_data)+len(processed_val_data)+len(processed_test_data)-1}")

if __name__ == '__main__':
    input_file = '../dataset/data/reveal.json'
    output_dir = '../dataset/data/reveal'
    process_reveal_dataset(input_file, output_dir)